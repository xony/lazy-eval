...
class MinHeuristic {
  ...
  public:
    MinHeuristic(const Graph& g) : _graph(g) {
	...
	/* the eleminatin order */
	    ...
	    // get first bucket with elements
            while (current_set->size() == 0)
                current_set = buckets[++minimum_degree];

//	    std::cout << "mindegree: " << minimum_degree <<std::endl;
        ...
        for(unsigned int i = 0; i < num_vertices; ++i) {
            ...
            for (unsigned int j=0; j < num_vertices-i; j++)
                bag->add(vertices[j]);
            ...
            if(i == 0) {
                ...
                //}
                //assert(i != j);
                tdc->add_edge(tree_nodes[i], tree_nodes[i-1]);
		//if(i=num_vertices-1)
		//    tdc->root(tree_nodes[i]);
            }
        }
